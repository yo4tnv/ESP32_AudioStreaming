...
void streamWav(WiFiClient &client) {
  ...
  while (client.connected()) {
    ...
    for (size_t i=0;i<PKT_FRAMES*CHANNELS;i++) {
      ...
      s = (int32_t)(((int64_t)s * g_gain_q15) >> 15); // +3 dB
      out[i] = clamp16(s);
    }
...
static void handleGainJson(WiFiClient &client) {
  ...
  client.println("Connection: close");
  client.println();
  client.print("{\"db\":"); client.print(g_gain_db); client.print("}");
}
...
void handleRoot(WiFiClient &client) {
  ...
  s += F("<div class='card'><h3>Amplification</h3><p><input id=\"gain\" type=\"range\" min=\"-24\" max=\"24\" step=\"1\"> <b><span id=\"gdb\">0</span> dB</b></p><small>Range: -24 to +24 dB (software gain)</small><script>async function loadG(){try{let r=await fetch('/gain.json',{cache:'no-store'});let j=await r.json();let s=document.getElementById('gain');let l=document.getElementById('gdb');s.value=j.db;l.textContent=j.db;}catch(e){}}let _gt=null;function postG(v){if(_gt)clearTimeout(_gt);_gt=setTimeout(()=>{fetch('/gain?db='+v).then(()=>{document.getElementById('gdb').textContent=v;});},200);}document.addEventListener('DOMContentLoaded',()=>{let s=document.getElementById('gain');s.addEventListener('input',e=>postG(e.target.value));loadG();});</script>");
  s += F("</body></html>");
...
void setup() {
  ...
  setGainDb(storedDb);Serial.begin(115200);
  delay(200);
  Serial.println();
